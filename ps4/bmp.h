 ...
 *
 * Function writes BMP header to opened stream. If stream is not open
 * (is `NULL`) or header is `NULL`, function returns `false`.
 *
 * @param stream opened stream, where the header will be written
 * @param header the header to write
 * @return `true`, if BMP header was saved successfully, `false` otherwise.
 */
bool write_bmp(FILE* stream, const struct bmp_image* header);
